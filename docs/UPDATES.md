...
应用默认更新源为 `Xiamol/jiligulu`。正式发布使用 `v0.6.1`，附件仅提供 release APK；远端最新版本以实际检查结果为准。
...
- 正式发布时保持 `applicationId` 与签名一致，增加 `versionCode`，并将 `versionName` 更新为与 Release 标签相同的三段版本号。当前正式包为 versionCode 11 / versionName 0.6.1，旧 0.5.6 可以检测到新版。
